...
(function(){
    this.initialize = function(attachable_type, attachable_id){
        // 建立新建表格的隱藏欄位，用來儲存上傳檔案的 id
        $('form.new_' + attachable_type.toLowerCase()).append( $("<input type='hidden' id='"+attachable_type.toLowerCase()+"_attachments' name='attachments'>") );
        ...
        $('#fileupload').fileupload({
            ...
            $.each(data.result.files, function (index, file) {
                // 設定上傳檔案的編號，讓新建表單可以在儲存成功後更新 attachable_id 的值
                var attachments_obj = $('#' + attachable_type.toLowerCase() + '_attachments');
                var attachments = attachments_obj.val();
                attachments_obj.val( attachments + ',' + file.id );
