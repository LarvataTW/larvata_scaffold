...
$(function() {
    ...
    if( typeof <%= singular_name %>_id !== 'undefined' && <%= singular_name %>_id != "" && typeof master_show_tab !== 'undefined' && master_show_tab != "" ) {
        $.ajax({
...
editable_attributes_and_except_sorting_and_datetime_and_number.each do |attr| 
  ...
  if belongs_to_assoc 
    ...
    assoc_plural_name = assoc_singular_name.pluralize
%>
    <%= singular_name %>_<%= attr.name %>_select2 = $('#<%= singular_name %>_<%= attr.name %>').is("select") && $('#<%= singular_name %>_<%= attr.name %>').select2({
        theme: 'bootstrap',
        width: '100%',
