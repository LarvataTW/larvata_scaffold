...
(function() {
    ...
    this.datatables.redraw = function(){
        submit_filter()
    };
    ...
    <% end -%>
                    ...
                    %>
                    {
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-file-text fa-lg"></span></a>';
                            actions += '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
    ...
    <% if contains_sorting_column? -%>
                    // 設定可以拖拉調整排序的欄位
                    $(row).find('td:not(".editable"):not(:last)').addClass('reorder');
    <% end -%>
