...
$(function() {
    ...
    // 列表頁的 datatables 設定
    $('.<%= plural_name %>_table').each(function(){
        var table = $(this);
        ...
        var editor = new $.fn.dataTable.Editor({
            ...
            table: "#"+ tab_key +"_<%= plural_name %>_table",
            idSrc: "DT_RowId",
            fields: [
...
<% end -%>
                ...
                %>
                {
                    ...
                    render: function ( data, type, full, meta ) {
                        var id = data.id;
                        var actions = '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '/edit" class="btn btn-default m-r-5"><span class="glyphicon glyphicon-pencil"></span></a>';
...
<% end -%>
        ...
        $("#"+ tab_key +"_<%= plural_name %>_table").on( 'click', 'tbody td.editable', function (e) {
            editor.inline( datatable.cell(this), {
                onBlur: 'submit'
            } );
        } );
