...
$(function() {
    ...
    $('.<%= controller_file_name %>_table').each(function(){
      ...
      newEditor.on('postSubmit', function(e, json, data, action, xhr) {
        if (json.success) {
...
<% end -%>
                ...
                %>
                {
                    ...
                    render: function ( data, type, full, meta ) {
                        ...
                        var actions = '<a data-id="' + id + '" href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '/edit?master_show_tab='+ master_show_tab_value +'" class="btn btn-default m-r-5 editButton"><span class="glyphicon glyphicon-pencil"></span></a>';
                        actions += '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '" data-confirm="確定刪除?" class="btn btn-default m-r-5" rel="nofollow" data-method="delete"><span class="glyphicon glyphicon-trash"></span></a>';
                        actions += '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '/rank_up" class="btn btn-default m-r-5 rankUP"><span class="glyphicon glyphicon-arrow-up"></span></a>';
                        actions += '<a href="<%= '/admin' if admin? %>/<%= controller_file_path %>/' + id + '/rank_down" class="btn btn-default m-r-5 rankUP"><span class="glyphicon glyphicon-arrow-down"></span></a>';
                        return actions;
                    },
                },
            ],
            ...
            buttons: [
                { extend: 'create', editor: newEditor, text: 'New', className: 'btn btn-default datatables_buttons' },
              /*
